...
(async function () {
  ...
  if (!("BarcodeDetector" in window)) {
    try {
      ...
      return toastAlert(
        "BarcodeDetector API is not supported by your browser.",
        "danger"
      );
    }
  } else {
    ...
    BookData.forEach((item) => {
      if (item.barcodeData) {
        item.barcodeData.forEach((barcode) => {
          if (
            barcode.scannedData &&
            barcode.scannedData.includes(searchBarcode)
          ) {
            found = true;
  ...
  function renderHistoryList(data) {
    ...
    if (!Array.isArray(data) || !data.length) {
      historyList.innerHTML =
        "<li class=>There are no saved items in history.</li>";
  ...
  async function createResult(value, resultDialog) {
    ...
    resultItem.classList.toggle(
      "results__item--no-barcode",
      value === NO_BARCODE_DETECTED
    );
    // resultItem.textContent = value  +"\n"+"order id:" + displayedDorderid +"\n"+ "barcodeCount" + await getCountFromAPI(searchBarcode);
    // resultItem.innerHTML = ` Barcode Value: ${value}<br /> ${displayedDorderid ? "Distributor Detail:"` ${displayedDorderid}` : "Not Found" }<br /> Distributor Detail: ${displayedDorderid}<br />Barcode Count: ${await getCountFromAPI(searchBarcode)} <br /> ${barcodeResult}`;
    // resultItem.innerHTML = `Barcode Value : ${value}<br />${displayedDorderid ? "Distributor Detail : " + displayedDorderid : "Distributor Detail : Not Found"}<br />Barcode Count : ${await getCountFromAPI(searchBarcode)}<br />${barcodeResult}`;
    // resultItem.innerHTML = `<span style="font-size: 24px ; font-weight: bold; color: ${barcodeResult === 'Verified' ? 'green' : 'red'}">${barcodeResult}</span> Barcode Value: ${value}<br />${displayedDorderid ? "Distributor Detail: " + displayedDorderid : "Distributor Detail: Not Found"}<br />Barcode Count: ${await getCountFromAPI(searchBarcode)}<br />`;

    resultItem.innerHTML = `
    <span style="font-size: 24px ; font-weight: bold; color: ${
      barcodeResult === "Verified" ? "green" : "red"
    }">${barcodeResult}</span>
    <table style="width: 70%; margin: auto;">
      ...
      <tr>
        ...
        <td>${displayedDorderid ? displayedDorderid : "Not Found"}</td>
      </tr>
      <tr>
        <td><strong>Barcode Count:</strong></td>
        <td>${await getCountFromAPI(searchBarcode)}</td>
      </tr>
    </table>`;

    // // Set font color based on barcodeResult
    // if (barcodeResult === "Verified") {
    //   resultItem.style.color = "white";
    // } else {
    //   resultItem.style.color = "red";
    // }

    resultDialog.insertBefore(
      resultItem,
      resultDialog.querySelector(".results__actions")
    );
  ...
  settingsForm.addEventListener("change", (evt) => {
    const settings = {};
    const checkboxes = evt.currentTarget.querySelectorAll(
      'input[type="checkbox"]'
    );
