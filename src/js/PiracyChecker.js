...
export default function PiracyBarcode(value) {
  ...
  async function fetchData() {
    try {
      const response1 = await fetch("https://nodei.ssccglpinnacle.com/getship");
      const responseData1 = await response1.json();
      data.push(...responseData1.reverse());
      console.log(data);

      const response2 = await fetch("https://nodei.ssccglpinnacle.com/getApproveDPO");
      const responseData2 = await response2.json();
      dataa.push(...responseData2.reverse());
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }

  async function postBarcodeToMongoDB() {
    try {
      await fetch("https://nodei.ssccglpinnacle.com/barcodeadd", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
